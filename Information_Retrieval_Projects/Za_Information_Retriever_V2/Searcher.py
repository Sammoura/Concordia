...
def main():
    ...
    titles = Indexer.readBlock('./inventory/docTitle.json')             
    bodies = Indexer.readBlock('./inventory/docBody.json')
    docLenIndex = Indexer.readBlock('./inventory/docTokenCount.json')          # (doc ID --> doc length) index ,retrieves index of documents lengths (number of tokens)
    l_ave  = getAverageLength(docLenIndex)
...
#       print(blockSize, "documents per block\n" )
    ...
    while searchChoice == 'y' :
        ...
        if len(tempResult) == 0:
            ...
            [queryFinalResult, freeQueryFinalResult] = Ranker.rankDocsForQuery(tempResult,docLenIndex, l_ave)
...
def getAverageLength(docLenIndex):
    totalLength = 0
    for value in docLenIndex:       # sum all doc lengths
        totalLength = totalLength + int(value)
    
    return totalLength/N            # return average
