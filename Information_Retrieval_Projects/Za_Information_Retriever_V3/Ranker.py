...
def docScoreOfTerm(termPostingEntry, docLenIndex, N , l_ave):
    ...
    for elt in termPostingEntry:
        ...
        for key in docLenIndex:
            ld = float(docLenIndex[key])      # document length
...
def rankDocsForQuery(List_Postings,docLenIndex, N , l_ave):         
    ...
    for termPosting in List_Postings:                                       # Processes one posting list of one query term at a time
        term_DocScore = docScoreOfTerm(termPosting, docLenIndex, N, l_ave)     # A dictionary of {docID --> doc score of a doc for a term } used by AND based query
